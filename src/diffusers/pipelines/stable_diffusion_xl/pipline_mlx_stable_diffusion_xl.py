...
class MLXStableDiffusionXLPipeline(FlaxDiffusionPipeline):
    def __init__(
        self,
    ...
    def _get_add_time_ids(
        self, original_size, crops_coords_top_left, target_size, bs, dtype
    ):
    ...
    def _generate(
        ...
        prompt_embeds = jnp.concatenate(
            [neg_prompt_embeds, prompt_embeds], axis=0
        )  # (2, 77, 2048)
        add_text_embeds = jnp.concatenate(
            [negative_pooled_embeds, pooled_embeds], axis=0
        )
