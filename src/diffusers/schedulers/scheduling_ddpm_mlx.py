...
class DDPMSchedulerState:
    ...
    def create(
        ...
        return cls(
            common=common, init_noise_sigma=init_noise_sigma, timesteps=timesteps
        )


@dataclass
class MLXDDPMSchedulerOutput(MLXSchedulerOutput):
...
class MLXDDPMScheduler(MLXSchedulerMixin, ConfigMixin):
    ...
    def create_state(
        ...
        init_noise_sigma = mx.array(1.0, dtype=self.dtype)
    ...
    def set_timesteps(
        ...
        Args:
            state (`DDIMSchedulerState`):
                the `MLXDDPMScheduler` state data class instance.
        ...
        timesteps = (mx.arange(0, num_inference_steps) * step_ratio).round()[::-1]
    ...
    def _get_variance(
        self, state: DDPMSchedulerState, t, predicted_variance=None, variance_type=None
    ):
        alpha_prod_t = state.common.alphas_cumprod[t]
        alpha_prod_t_prev = mx.where(
            t > 0, state.common.alphas_cumprod[t - 1], mx.array(1.0, dtype=self.dtype)
        )
        ...
        if variance_type == "fixed_small":
            variance = mx.clip(variance, a_min=1e-20, a_max=None)
        # for rl-diffuser https://arxiv.org/abs/2205.09991
        elif variance_type == "fixed_small_log":
            variance = mx.log(mx.clip(variance, a_min=1e-20, a_max=None))
        elif variance_type == "fixed_large":
            variance = state.common.betas[t]
        elif variance_type == "fixed_large_log":
            # Glide max_log
            variance = mx.log(state.common.betas[t])
    ...
    def step(
        ...
        Args:
            ...
            key (`mx.array`): a PRNG key.
            return_dict (`bool`): option for returning tuple rather than MLXDDPMSchedulerOutput class

        Returns:
            [`MLXDDPMSchedulerOutput`] or `tuple`: [`MLXDDPMSchedulerOutput`] if `return_dict` is True, otherwise a
        ...
        if (
            ...
            model_output, predicted_variance = mx.split(
        ...
        # 6. Add noise
        def random_variance():
            split_key = mx.random.split(key, num=1)[0]
            noise = mx.random.normal(
                shape=model_output.shape, dtype=self.dtype, key=split_key
            )
            return (
        ...
        variance = mx.where(
            t > 0, random_variance(), mx.zeros(model_output.shape, dtype=self.dtype)
        )
        ...
        return MLXDDPMSchedulerOutput(prev_sample=pred_prev_sample, state=state)
